...
import 'package:ethosv2/feature/contact/controllers/contacts_controller.dart';
...
class ContactPage extends ConsumerWidget {
  ...
  shareSmsLink(phoneNumber) async {
    Uri sms = Uri.parse(
      "sms:$phoneNumber?body=Let's chat on CYLO! it's a fast, simple, and secure app we can call each other for free. Get it at https://CYLO.com/dl/",
    );
        ...
        title: Column(
          ...
          children: [
            ...
            ref.watch(contactsControllerProvider).when(
                data: (allContacts){
                  return Text("${allContacts[0].length} Contact${allContacts[0].length ==1 ? '': 's'}", style: const TextStyle(fontSize: 13),);
                },
                error: (e,t){
